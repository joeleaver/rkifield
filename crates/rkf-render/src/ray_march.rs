...
impl RayMarchPass {
    ...
    pub fn new(device: &wgpu::Device, scene: &GpuScene, gbuffer: &GBuffer) -> Self {
        ...
        // Pipeline layout: group 0 = scene data, group 1 = G-buffer write
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("ray march pipeline layout"),
            bind_group_layouts: &[&scene.bind_group_layout, &gbuffer.write_bind_group_layout],
    ...
    pub fn dispatch(&self, encoder: &mut wgpu::CommandEncoder, scene: &GpuScene, gbuffer: &GBuffer) {
